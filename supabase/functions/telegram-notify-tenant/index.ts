    ...
    if (action === 'approve') {
      ...
      if (!landlordId) {
        await sendMessage(BOT_TOKEN, chatId,
          `Hey ${firstName}! Great news — the landlord loved your application for <b>${address}</b> 🎉 They'll be in touch about a viewing soon!`
        );
      ...
      if (!schedule || schedule.length === 0) {
        await sendMessage(BOT_TOKEN, chatId,
          `Hey ${firstName}! The landlord really liked your profile for <b>${address}</b> 🎉 They're setting up viewing times — I'll send you options as soon as they're ready!`
        );
        return ok(corsHeaders);
      }
      ...
      if (slots.length === 0) {
        await sendMessage(BOT_TOKEN, chatId,
          `Hey ${firstName}! Your application for <b>${address}</b> was approved 🎉 All viewing slots are taken right now, but I'll message you the moment one opens up!`
        );
        return ok(corsHeaders);
      }
